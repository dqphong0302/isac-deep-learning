% compare_v36_simple.m
% -------------------------------------------------------------------------
% V3.6 OPTIMIZED: Based on working V2 script
% Changes from V2:
%   - Np_pilot_dl = 4 (instead of 8)
%   - N_pilot = 4*Nc (for GSFDIC)
%   - Ridge LS with lambda=0.1
%   - Uses v36_opt.onnx
% -------------------------------------------------------------------------
...
% === V3.6 KEY PARAM ===
Np_pilot_dl = 4;         % V3.6 uses 4 pilots (V2 used 8)
...
%% Params
hcom_length = 4;
N_pilot = Np_pilot_dl * Nc;  % GSFDIC uses same pilot count as DL
...
%% Monte Carlo
fprintf('\n========================================\n');
fprintf('V3.6 Optimized: 4 Pilots + Ridge LS\n');
...
fprintf('\n');
fprintf('╔══════════════════════════════════════════════════════════════════╗\n');
fprintf('║       V3.6 OPTIMIZED: 4 PILOTS + RIDGE LS COMPARISON            ║\n');
fprintf('╠══════════════════════════════════════════════════════════════════╣\n');
fprintf('║                        NMSE (dB) Statistics                       ║\n');
fprintf('╠═══════════════════╦══════════════════╦═══════════════════════════╣\n');
fprintf('║     Metric        ║  GSFDIC (4 pil)  ║   V3.6 DL (4 pil)         ║\n');
fprintf('╠═══════════════════╬══════════════════╬═══════════════════════════╣\n');
fprintf('║  Mean             ║    %+8.2f dB    ║       %+8.2f dB          ║\n', mean(nmse_gsfdic), mean(nmse_v36));
fprintf('║  Median           ║    %+8.2f dB    ║       %+8.2f dB          ║\n', median(nmse_gsfdic), median(nmse_v36));
fprintf('║  Std Dev          ║    %8.2f dB    ║       %8.2f dB          ║\n', std(nmse_gsfdic), std(nmse_v36));
fprintf('║  Min (Best)       ║    %+8.2f dB    ║       %+8.2f dB          ║\n', min(nmse_gsfdic), min(nmse_v36));
fprintf('║  Max (Worst)      ║    %+8.2f dB    ║       %+8.2f dB          ║\n', max(nmse_gsfdic), max(nmse_v36));
fprintf('╚═══════════════════╩══════════════════╩═══════════════════════════╝\n');

improvement = mean(nmse_gsfdic) - mean(nmse_v36);
win_rate = sum(nmse_v36 < nmse_gsfdic) / Nmc * 100;

fprintf('\n');
fprintf('╔══════════════════════════════════════════════════════════════════╗\n');
fprintf('║                       IMPROVEMENT ANALYSIS                        ║\n');
fprintf('╠══════════════════════════════════════════════════════════════════╣\n');
fprintf('║  V3.6 DL improves over GSFDIC by:    %+.2f dB                    ║\n', improvement);
fprintf('║  V3.6 DL wins in:                    %.1f%% of trials             ║\n', win_rate);
fprintf('╚══════════════════════════════════════════════════════════════════╝\n');

[~, pval] = ttest(nmse_gsfdic, nmse_v36);
fprintf('\n');
fprintf('╔══════════════════════════════════════════════════════════════════╗\n');
...
plot(sort(nmse_v36), (1:Nmc)/Nmc, 'r--', 'LineWidth', 2.5);
legend({'GSFDIC (4 pilots)', 'V3.6 DL (4 pilots)'}, 'Location', 'best');
xlabel('NMSE (dB)'); ylabel('CDF');
title('V3.6: CDF of Channel Estimation NMSE');
saveas(fig1, fullfile(figFolder, ['cdf_v36_' timestamp '.png']));

fig2 = figure('Position', [100 650 600 400]);
boxplot([nmse_gsfdic, nmse_v36], 'Labels', {'GSFDIC', 'V3.6 DL'});
ylabel('NMSE (dB)'); title('V3.6: Distribution Comparison'); grid on;
saveas(fig2, fullfile(figFolder, ['boxplot_v36_' timestamp '.png']));
