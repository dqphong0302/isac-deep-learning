function [ decVec, QF_tem,dec_tem] = ldpcDecode( llrVec, LDPC )
...
Rcf = spalloc(LDPC.numParBits, LDPC.numTotBits + 2*LDPC.Z, numEntries);

QF_tem = zeros(LDPC.iterations,LDPC.numTotBits + 2*LDPC.Z);
dec_tem = zeros(LDPC.iterations,LDPC.numInfBits);
    ...
    case 'SPA' % Sum-Product Algorithm
        ...
        for ldpcCurIter = 1:LDPC.iterations
            ...
            for checkIdx = 1:LDPC.numParBits
                ...
                % Tmp update llr
                tmpLlr = Qf(nbVarNodes) - full(Rcf(checkIdx,nbVarNodes));
                ...
                for varIter = 1:length(nbVarNodes)
                    ...
                    Qtmp = Qf(varIdx) - Rcf(checkIdx, varIdx);
                    ...
                    Rcf(checkIdx, varIdx) = Ssign*QtmpSign * (-log(minVal+tanh(abs(Smag-QtmpMag)/2)));
                    
                    % Update Qv. From reference: Qv = Qtmp + Rcv
                    Qf(varIdx)  = Qtmp + Rcf(checkIdx, varIdx);
                    
                end
                
            end
            QF_tem(ldpcCurIter,:)= Qf;
            dec_tem(ldpcCurIter,Qf(1:LDPC.numInfBits)<0) = 1;
           
        end
        
        % Convert Qv to decoded bits
        decVec = zeros(1,LDPC.numInfBits);
        decVec(Qf(1:LDPC.numInfBits)<0) = 1;
