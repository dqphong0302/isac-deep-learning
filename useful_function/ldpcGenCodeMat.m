function [ LDPC ] = ldpcGenCodeMat( rateIn,kb, bgType ,liftingFactor)
...
% Work a round for nice code rate specifications
...
tmpBG = load(['prototypes\BG2_ILS', num2str(iLs), '.mat']);

%% Modify base graph to desired code rate

% Find N to shorten BG accordingly
for N = 0:(mb-4)
    if LDPC.rate - maxCodeRateDeviation < kb/(nb-N-2) && kb/(nb-N-2) < LDPC.rate + maxCodeRateDeviation
        break;
    elseif N == (mb-4)
...
if LDPC.rate < 1/5 + maxCodeRateDeviation
    rateStr = '1_5';
elseif LDPC.rate < 1/4 + maxCodeRateDeviation
    rateStr = '1_4';    
elseif LDPC.rate < 1/3 + maxCodeRateDeviation
    rateStr = '1_3';
