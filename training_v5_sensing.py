#!/usr/bin/env python3
"""
training_v5_sensing.py
V5: Sensing-Assisted Channel Estimation

Uses radar sensing results (detected targets) as additional features.
Input: 227 features (210 base + 17 sensing)
"""
...
class SensingAssistedMLP(nn.Module):
    """MLP with sensing-assisted features"""
    
    def __init__(self, in_dim=227, width=512, depth=4, dropout=0.2):
        super().__init__()
        layers = []
        d = in_dim
...
class V5Dataset(Dataset):
    def __init__(self, h5_path):
        with h5py.File(h5_path, 'r') as f:
            ...
            # h_ls is at positions 202:210 (8 features before sensing)
            self.Hrls = self.X[:, 202:210]
            self.n_samples, self.in_dim = self.X.shape
            
            print(f"Dataset: {self.n_samples} samples, {self.in_dim} features")
            if '/meta/sensing' in f:
                print("Dataset type: Sensing-Assisted")
...
def main():
    ...
    parser.add_argument("--h5", default="dataset_v5_sensing.h5")
    ...
    parser.add_argument("--ckpt", default="best_v5_sensing.pt")
    ...
    model = SensingAssistedMLP(in_dim=in_dim, width=args.width, depth=args.depth, dropout=args.dropout).to(device)
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Model: SensingAssistedMLP, {n_params:,} params")
    ...
    print("\n" + "="*60)
    print("Training V5: Sensing-Assisted Channel Estimation")
    print("="*60 + "\n")
    ...
    for ep in range(1, args.epochs+1):
        ...
        if val_nmse < best_nmse:
            ...
            torch.save({
                ...
                'in_dim': in_dim, 'width': args.width, 'depth': args.depth,
                'val_nmse': val_nmse
            }, args.ckpt)
            saved = " *"
