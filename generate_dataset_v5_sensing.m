% generate_dataset_v5_sensing_assisted.m
% -------------------------------------------------------------------------
% V5: Sensing-Assisted Channel Estimation
%
% Key innovation: Add radar sensing results as features
% - Detected target info (range, velocity, power)
% - Residual interference level after FDIC
% - Combined with V3.6 features (spectral + h_ls)
% -------------------------------------------------------------------------
...
% Sensing features: max 5 targets x 3 features (range, velocity, power) + padding
maxTargets = 5;
sensingDim = maxTargets * 3 + 2;  % +2 for num_targets and residual_power = 17

% Total features: 192 (spectral) + 9 (global) + 1 (norm) + 8 (h_ls) + 17 (sensing) = 227
xDim = 3*B + G + 1 + 2*hcom_length + sensingDim;  % 227
yDim = 2*hcom_length;

Np_pilot = 4;
Ridge_Lambda = 0.1;

outFile = 'dataset_v5_sensing.h5';
...
h5create(outFile, '/meta/version', 1, 'Datatype','int32'); h5write(outFile,'/meta/version', int32(5));
h5create(outFile, '/meta/sensing', 1, 'Datatype','int32'); h5write(outFile,'/meta/sensing', int32(1));
...
%% Generate samples
fprintf('=== V5: Sensing-Assisted Dataset ===\n');
fprintf('Features: %d (spectral + sensing)\n', xDim);
fprintf('Samples: %d\n\n', numSamples);
tic;
...
for idx = 1:numSamples
    ...
    RDM = calc_periodogram(E, Ftx_radar, 1/100, 1/100);
    try
        targetout = cfar_ofdm_radar(RDM, numTarget, 1, 3, 3, 3, 3);
        detected_ranges = targetout.ranges;
        detected_velocities = targetout.velocities;
        detected_powers = targetout.powers;
        num_detected = length(detected_ranges);
    catch
        detected_ranges = [];
        detected_velocities = [];
        detected_powers = [];
    ...
    % Pad sensing features to fixed size
    sensing_ranges = zeros(maxTargets, 1);
    sensing_velocities = zeros(maxTargets, 1);
    sensing_powers = zeros(maxTargets, 1);
    
    for t = 1:min(num_detected, maxTargets)
        sensing_ranges(t) = detected_ranges(t) / r_max;  % Normalize
        sensing_velocities(t) = detected_velocities(t) / v_max;
        if ~isempty(detected_powers)
            sensing_powers(t) = detected_powers(t) / 100;  % Rough normalize
        end
    end
    
    % Residual power after FDIC (sensing info)
    residual_power = 10*log10(mean(abs(E(:)).^2) + eps) / 50;  % Normalize
    
    % Sensing feature vector
    sensing_feat = single([num_detected/maxTargets; sensing_ranges; sensing_velocities; sensing_powers; residual_power]);
    % =====================================

    % Standard V3.6 features
    ...
    % Combine all features: spectral (202) + sensing (17) = 227? Actually 210 + 17 = 227
    x_in = single([band_mean; band_std; band_max; g; nf_feat; hrls8; sensing_feat]);
...
fprintf('Feature dim: %d (210 base + 17 sensing)\n', xDim);
