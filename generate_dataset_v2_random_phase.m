% generate_dataset_v2_random_phase.m
% -------------------------------------------------------------------------
% V2: Semi-Blind with Random Phase
%
% Key differences:
%   - RANDOM PHASE: Different each sample (prevents overfitting)
%   - WITH h_ls: Includes LS estimate from pilot (210 features)
%
% This tests if model can generalize with random phase
% when it has h_ls as a "hint"
% -------------------------------------------------------------------------
...
% Feature config (WITH h_ls!)
B = 64;
bandSize = Nc / B;
percentiles = [10 25 50 75 90];
G = 4 + numel(percentiles);
xDim = 3*B + G + 1 + 2*hcom_length;  % 192 + 9 + 1 + 8 = 210 (with h_ls)
yDim = 2*hcom_length;

Np_pilot = 8;

outFile = 'dataset_v2_random_phase.h5';
...
h5create(outFile, '/meta/has_hls', 1, 'Datatype','int32'); h5write(outFile,'/meta/has_hls', int32(1));
...
%% Generate samples
fprintf('=== V2: Semi-Blind with Random Phase ===\n');
fprintf('Features: 210 (WITH h_ls from pilot)\n');
...
for idx = 1:numSamples
    ...
    % LS estimate from pilots (THIS IS THE KEY DIFFERENCE FROM V3)
    e_td = DFT_matrix' * E;
    ...
    % Feature: 192 + 9 + 1 + 8 = 210 (WITH h_ls)
...
fprintf('Feature dim: %d (WITH h_ls)\n', xDim);
fprintf('Saved to: %s\n', outFile);
